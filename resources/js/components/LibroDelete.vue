<template>
    ...
    <button class="btn btn-danger btn-sm" id="eliminar" @click="eliminar(libro)"><i class="fas fa-trash-alt"></i></button>
</template>
        ...
        eliminar(libro){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete(`/biblioteca/libros/${libro.isbn}`)
                    .then(function (response) {
                        ...
                        if(response.status == 200){
                            ...
                            const renderizar = document.querySelectorAll(`[data-isbn="${response.data.isbn}"]`)
                            //se obtiene un array, se le pasa la posicion 0 para que sea eliminado del dom
                            renderizar[0].remove()
                            toastr.success('Libro se ha eliminado', 'Eliminado')
                        }else{
